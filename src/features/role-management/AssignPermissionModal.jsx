  ...
  return (
    <Modal
      ...
      footer={
        <div className="flex items-center justify-between w-full">
          <div className="text-xs text-[#52677A] font-medium">
            <span className="font-mono font-bold text-[#14804A]">{selectedActionIds.size}</span> of{' '}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-8.5 pl-9 pr-3 text-xs bg-[#F4F7FA] border border-[#D8E0E8] rounded-md text-[#172B4D] outline-hidden focus:border-[#147D83]"
            />
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <Button
              ...
              className="text-xs h-8 text-[#748597]"
            >
              Deselect All
            </Button>
          </div>
        ...
        <div className="border border-[#D8E0E8] rounded-lg overflow-hidden divide-y divide-[#D8E0E8] max-h-[460px] overflow-y-auto bg-white">
          {parents.map((parent) => {
            ...
            return (
              <div key={parent.id} className="bg-white">
                ...
                <div className="px-4 py-2.5 bg-[#F8FAFC] flex items-center justify-between border-b border-[#EAEFF5] hover:bg-[#F1F5F9] transition-colors">
                  <div
                    onClick={() => toggleParentExpand(parent.id)}
                    className="flex items-center gap-2 cursor-pointer select-none flex-1 truncate"
                  >
                    {isExpanded ? (
                      <ChevronDown className="w-4 h-4 text-[#748597] shrink-0" />
                    ) : (
                      <ChevronRight className="w-4 h-4 text-[#748597] shrink-0" />
                    )}
                    <FolderTree className="w-4 h-4 text-[#147D83] shrink-0" />
                    <span className="text-xs font-bold text-[#102A43] truncate">
                      {parent.name}
                    </span>
                    <span className="text-[11px] text-[#748597] font-mono">
                      ({selectedParentActions.length}/{allParentActions.length})
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="text-[11px] font-semibold text-[#147D83] hover:underline cursor-pointer"
                    >
                ...
                {isExpanded && (
                  <div className="p-3 pl-8 space-y-3 bg-[#FAFCFE]">
                    {parentPerms.map((perm) => {
                      ...
                      return (
                        <div
                          key={perm.id}
                          className="border border-[#EAEFF5] rounded-md bg-white p-3 space-y-2"
                        >
                          {/* Level 2: Permission Node */}
                          <div className="flex items-center gap-2 pb-1.5 border-b border-[#F4F7FA]">
                            <KeyRound className="w-3.5 h-3.5 text-[#10683D] shrink-0" />
                            <span className="text-xs font-semibold text-[#172B4D]">
                              {perm.name}
                            </span>
                            <span className="text-[11px] font-mono text-[#748597] ml-auto">
                              {perm.path}
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 pl-4">
                            {actionsForPerm.length === 0 ? (
                              <span className="text-[11px] text-[#748597] italic">
                                ...
                                return (
                                  <label
                                    ...
                                    className={`flex items-start gap-2 p-1.5 rounded border transition-colors cursor-pointer text-xs ${
                                      isChecked
                                        ? 'bg-[#14804A]/5 border-[#14804A]/30 text-[#10683D]'
                                        : 'bg-white border-[#D8E0E8] text-[#172B4D] hover:bg-[#F4F7FA]'
                                    }`}
                                  >
                                    <input
                                      type="checkbox"
                                      ...
                                      className="rounded border-[#D8E0E8] text-[#14804A] mt-0.5"
                                    />
                                    <div className="truncate">
                                      <p className="font-medium truncate">{action.name}</p>
                                      <div className="flex items-center gap-1.5 text-[10px] font-mono text-[#748597] mt-0.5">
                                        <span className="font-bold uppercase text-[#147D83]">
                                          {action.method}
                                        </span>
